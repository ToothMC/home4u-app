-- 0050_index_cy_source_and_generic_upsert.sql
--
-- 1. listing_source-Enum erweitern um 'index_cy'.
-- 2. Generische bulk_upsert_external_listings(p_source, p_rows) RPC
--    — ersetzt langfristig source-spezifische RPCs (bazaraki, fb).
--    Identische Dedup-Logic wie bulk_upsert_bazaraki_listings (0049):
--    pHash-Schreiben, find_canonical_for_signals, canonical_id setzen.
--
-- Hinweis: ALTER TYPE ... ADD VALUE muss in eigener Transaktion vor der
-- RPC-Definition committet werden — daher zwei separate Migrations-Calls.

alter type listing_source add value if not exists 'index_cy';
