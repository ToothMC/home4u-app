-- 20260502120000_cluster_similar_same_location.sql
--
-- Cluster-Anzeige Re-Enable mit "ähnlich"-Framing:
-- - get_cluster_offers + match_listings_for_profile filtern beide auf
--   gleichen Ortsteil (location_district) + gleichen type + gleiche
--   rooms relativ zum aktuell betrachteten Listing.
-- - KEIN Preis-Filter: User darf günstigere/teurere ähnliche Inserate
--   sehen, das ist die ursprüngliche Spec.
-- - Frontend zeigt jetzt "{n} ähnliche Inserate" statt "auch bei N
--   anderen Anbietern" — keine falsche Behauptung mehr dass es dieselbe
--   Wohnung ist.
--
-- Spec-Klarstellung: cluster_offers_count im match-RPC zählt nur Cluster-
-- Member die zum Match-Listing auf district + type + rooms passen.
-- Stockfoto-Cluster die quer über Districts / Zimmer-Anzahl / type
-- spannen werden für jedes Member als Singleton (count=1) gezeigt.

-- 1. get_cluster_offers: nur ähnliche am gleichen Ort
...
begin
  select coalesce(l.canonical_id, l.id),
         l.location_city, l.location_district, l.type, l.rooms
    into v_master, v_city, v_district, v_type, v_rooms
  ...
  where l.status = 'active'
    ...
    and l.location_city = v_city
    and (
      (l.location_district is null and v_district is null)
      or l.location_district = v_district
    )
    and l.type = v_type
    and coalesce(l.rooms, -1) = coalesce(v_rooms, -1)
...
-- 2. match_listings_for_profile: cluster_stats per Listing zählt nur
--    Cluster-Member die District+type+rooms+city teilen.
...
begin
  ...
  -- Per Candidate: zähle ähnliche Listings im selben Cluster mit
  -- gleichem city/district/type/rooms. Stockfoto-Cluster werden so
  -- automatisch zerlegt — Members in anderen Districts zählen nicht mit.
