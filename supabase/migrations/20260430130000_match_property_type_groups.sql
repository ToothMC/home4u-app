-- 20260430130000_match_property_type_groups.sql
--
-- Match: property_type-Filter lockern auf semantische Gruppen.
-- Hintergrund: Vision differenziert das, was der Crawler pauschal als 'house'
-- flagged, in 'villa' / 'townhouse' / 'bungalow' / 'maisonette' /
-- 'penthouse' (alle keine Apartments). User geben oft den Top-Begriff
-- 'house' im Profil ein. Strikt-Match würde dann Villas etc. ausschließen.
--
-- Mapping:
--   profile=house  → matched: house, villa, townhouse, bungalow, maisonette, penthouse
--   profile=apartment → matched: apartment, studio
--   profile=plot/land → bidirektional plot↔land
--   profile=null → kein Filter
--
-- Plus: match_listings_for_profile auf den Helper umgestellt.

create or replace function private.match_property_type_compatible(
  p_profile_type text,
  p_listing_type text
)
returns boolean
...
as $$
  select case
    when p_profile_type is null then true
    when p_listing_type is null then true
    when p_profile_type = p_listing_type then true
    when p_profile_type = 'house' and p_listing_type in (
      'villa','townhouse','bungalow','maisonette','penthouse'
    ) then true
    when p_profile_type = 'apartment' and p_listing_type = 'studio' then true
    when (p_profile_type = 'plot' and p_listing_type = 'land')
      or (p_profile_type = 'land' and p_listing_type = 'plot') then true
    else false
  end;
$$;

revoke all on function private.match_property_type_compatible(text, text) from public, anon, authenticated;

-- match_listings_for_profile: property_type-Filter auf Helper umstellen.
-- Body sonst identisch zu 20260430120000_match_amenity_soft_boost.
-- (Hier komplett neu definiert weil create or replace die ganze Body braucht.)
