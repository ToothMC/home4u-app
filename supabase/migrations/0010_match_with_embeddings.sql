-- Match-RPC v2: kombiniert Hard-Filter + Preis/Zimmer-Score mit Cosine-Similarity
-- aus pgvector-Embeddings (text-embedding-3-small, 1536 Dim).
-- Whitepaper §3.6: Hard Filter (SQL) → Soft Rank (Embeddings) → Top-N.
--
-- Score-Komposition:
--   * Wenn beide Embeddings vorhanden:
--       0.50 * cosine_sim + 0.30 * price_proximity + 0.20 * room_match
--   * Sonst (Backward-Compat):
--       0.70 * price_proximity + 0.30 * room_match
-- Cosine-Sim wird auf [0,1] gemappt: (1 - cosine_distance) / 2 + 0.5
-- — bei pgvector ist `<=>` Distance (0 = identisch, 2 = gegenüber).
...
begin
  ...
  with candidates as (
    select
      ...
      l.size_sqm,
      l.contact_channel,
      -- Preis-Proximity (0..1): 1 bei exakter Übereinstimmung mit budget_max,
      -- linear fallend bis 0
      greatest(
        0,
      ...
      -- Cosine-Similarity (0..1) — nur wenn beide Embeddings da
      case
