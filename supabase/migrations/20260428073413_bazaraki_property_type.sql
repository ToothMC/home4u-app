-- bulk_upsert_bazaraki_listings: erweitert um property_type-Mapping.
--
-- Vorher (Migration 0029): listings.property_type blieb für Bazaraki-Source
-- immer NULL, weil die RPC kein property_type aus dem Row-Payload las.
-- Jetzt: Caller (bazaraki-crawler) liefert "property_type" als Home4U-
-- Taxonomie (apartment|house|room|plot — siehe config.HOME4U_PROPERTY_TYPE_BY_SUBTYPE),
-- die RPC schreibt es 1:1 in listings.property_type.
--
-- Sticky-Pattern beim Update: leerer Wert überschreibt nicht (coalesce),
-- analog zu title/description/energy_class.
