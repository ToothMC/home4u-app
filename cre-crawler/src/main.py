"""cyprus-real.estate-Crawler Entry-Point.

Flow analog index-crawler:
  1. Sitemap-Discovery: alle Listing-URLs aus 5 sub-sitemaps
...
sobald Wall-Clock das Budget reißt. Workflow-timeout sollte +10min Headroom
bieten, dann liefert Python sauber exit 0 statt SIGTERM. Mehrere kürzere
Runs pro Tag (cron alle paar Stunden) decken Backfill inkrementell ab.
...
def main() -> int:
    load_dotenv()
    _setup_logging()
    log = logging.getLogger("cre-crawler")
    ...
    with httpx.Client(headers=headers, follow_redirects=True) as client:
        ...
        # ein. Health-Check + mark_stale brechen dann mit p_min_recent_seen-
        # Guard ab (Inzident 2026-05-07: 104/1181 in 24h gesehen).
        # Nur wenn discover() URLs geliefert hat — bei leerem discover NICHT
